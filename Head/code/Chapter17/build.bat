...
if "%1" == "clean" del *.obj *.exe *.pdb *.idb *.intermediate.manifest build.log *.out > nul 2>&1 && goto END
if "%1" == "test"  goto TEST
...
if not "%VCINSTALLDIR%"=="" goto END
if "%1" == "msvc90" call "%VS90COMNTOOLS%vsvars32.bat" > build.log && echo Using MS Visual C++ 9.0 && set FLTKSlnDir=vc2008
if "%1" == "msvc80" call "%VS80COMNTOOLS%vsvars32.bat" > build.log && echo Using MS Visual C++ 8.0 && set FLTKSlnDir=vc2005
if "%1" == "msvc71" call "%VS71COMNTOOLS%vsvars32.bat" > build.log && echo Using MS Visual C++ 7.1 && set FLTKSlnDir=vcnet
goto END
...
rem Check if the user has provided path to FLTK and if it has been built. Builds a local one if not.
if "%FLTK%" == "" echo WARNING: FLTK environment variable has not been set. && if exist ..\FLTK set FLTK=..\FLTK&& echo Using provided copy of FLTK.
if "%FLTK%" == "" echo ERROR: Unable to locate FLTK directory && goto END
if not exist "%FLTK%\lib\fltk.lib" echo Building FLTK ... && "%DevEnvDir%\devenv.exe" /Build Release "%FLTK%\%FLTKSlnDir%\fltk.sln"
if not exist "%FLTK%\lib\fltk.lib" echo ERROR: There was a problem building FLTK && goto END
